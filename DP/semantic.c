...
static void sem_error(int lineno, const char *fmt, ...) {
    ...
    char fullMsg[1200];
    snprintf(fullMsg, sizeof(fullMsg), "Semantic Error [line %d]: %s",
             lineno, buffer);
    ...
    switch (node->kind) {
        case NODE_FUNC_DECL: {
            if (symtable_insert(curScope, node->name,
                ...
                /* local vars in function body - assign stack offsets */
                AST *body = node->extra;
                if (body && body->child) {
                    for (AST *st = body->child; st; st = st->sibling) {
                        if (st->kind == NODE_VAR_DECL) {
                            if (symtable_insert(fnScope, st->name,
                                ...
                                /* assign stack offset for local variable */
                                Symbol *sym = symtable_lookup(fnScope, st->name);
                                if (sym) {
                                    sym->size = symtable_type_size(st->typeName);
                                    sym->offset = fnScope->next_offset;
                                    fnScope->next_offset += sym->size;
                                    fnScope->frame_size = fnScope->next_offset - 4;  /* total frame size */
                                }
                            }
                        }
                    }
        ...
        case NODE_VAR_DECL: {
            if (symtable_insert(curScope, node->name,
                ...
                /* assign stack offset for variable (if in function scope) */
                Symbol *sym = symtable_lookup(curScope, node->name);
                if (sym && curScope->scopeName && strcmp(curScope->scopeName, "global") != 0) {
                    sym->size = symtable_type_size(node->typeName);
                    sym->offset = curScope->next_offset;
                    curScope->next_offset += sym->size;
                    curScope->frame_size = curScope->next_offset - 4;
                }
            }
            break;
        }
...
static const char *resolve_type_of_expr(SymTable *curScope, AST *expr) {
    ...
    switch (expr->kind) {
        ...
        case NODE_BINARY_OP: {
            ...
            const char *rt = resolve_type_of_expr(
                curScope, expr->child ? expr->child->sibling : NULL);
